...
version = "testing10runs"
#directory_path = f"files/3d_large/{version}"
...
n = 50*50*50
n_dim = [50,50,50]
m_choices = [int(n*0.03), int(n*0.04), int(n*0.08), int(n*0.12)]
num_choices = [1000,1000,2000,2000]
...
for i in range(len(m_choices)):
    ...
    A_4 = linalg.khatri_rao(A_0,linalg.khatri_rao(A_1,A_2))
    A = A_4.T
    b = A@x
    #start = time.time()
...
data_array =  pd.DataFrame(data_array_atiht)
data_array.to_csv(f"data_atiht_{idx}.csv")
#data_array =  pd.DataFrame(data_array_kztiht)
#data_array.to_csv(f"data_kztiht_{idx}.csv")
#data_array =  pd.DataFrame(data_array_tiht)
#data_array.to_csv(f"data_tiht_{idx}.csv")
#pd.DataFrame(time_array).to_csv(f"time_{idx}.csv")
